...
protected:
    ...
    double test_statistic(std::vector<double> const &samples, F &&perfect_distribution) {
        ...
        for (auto sm: multiplicities) {
            ...
            test_statistic = std::max(test_statistic,
                std::abs(perfect_distribution(sample - EPSILON) - static_cast<double>(accumulated_multiplicity) / samples.size()));
    ...
    void check_kolmogorov_smirnov(std::vector<double> const &samples, F &&perfect_distribution) {
        ...
        EXPECT_PRED_FORMAT2(::testing::DoubleLE,
            test_statistic(samples, perfect_distribution), max_test_statistic);
    }
};
...
TEST_F(RandomTest, test_that_the_test_statistic_is_properly_computed) {
    EXPECT_PRED_FORMAT2(::testing::DoubleLE,
        test_statistic({0.5}, uniform_zero_one_continuous_distribution),
        0.5);
    EXPECT_PRED_FORMAT2(::testing::DoubleLE,
        test_statistic({0.5, 0.7}, uniform_zero_one_continuous_distribution),
        0.5);
    EXPECT_PRED_FORMAT2(::testing::DoubleLE,
        test_statistic({0.1, 0.5, 0.7, 0.8}, uniform_zero_one_continuous_distribution),
        0.25);
    EXPECT_PRED_FORMAT2(::testing::DoubleLE,
...
TEST_F(RandomTestFirstSeedTest, kolmogorov_smirnov_test_for_random_integer) {
    ...
    check_kolmogorov_smirnov(samples,
        [&min, &max](double x) { return uniform_min_max_integer_distribution(min, max, x); });
}
