...
protected:
    ...
    TreeOne<CqasmV3xVariable> qubit_register_of_size_2 =
        cqasm_tree::make<CqasmV3xVariable>("qr_of_size_2", cqasm_tree::make<CqasmV3xQubitType>(2));
    TreeOne<CqasmV3xVariable> qubit_register_of_size_4 =
        cqasm_tree::make<CqasmV3xVariable>("qr_of_size_4", cqasm_tree::make<CqasmV3xQubitType>(4));
...
TEST_F(RegisterManagerTest, qubit_register_size_exceeds_maximum_allowed) {
    EXPECT_THAT(
        [this]() { [[maybe_unused]] auto qubit_register = QubitRegister{ program_with_too_long_qubit_register }; },
        ::testing::ThrowsMessage<std::runtime_error>(fmt::format("register size exceeds maximum allowed: {} > {}",
            qx::config::MAX_QUBIT_NUMBER + 1,
            qx::config::MAX_QUBIT_NUMBER)));
}

TEST_F(RegisterManagerTest, bit_register_size_exceeds_maximum_allowed) {
    EXPECT_THAT([this]() { [[maybe_unused]] auto bit_register = BitRegister{ program_with_too_long_bit_register }; },
        ::testing::ThrowsMessage<std::runtime_error>(fmt::format("register size exceeds maximum allowed: {} > {}",
            qx::config::MAX_BIT_NUMBER + 1,
            qx::config::MAX_BIT_NUMBER)));
}

TEST_F(RegisterManagerTest, get_instance_before_create_instance) {
    EXPECT_THAT([]() { [[maybe_unused]] const auto& unused = RegisterManager::get_instance(); },
