...
namespace qx
{
   ...
   class ninja_star_17q
   {
	 ...
	 */
	void correct_bitflips(uint32_t round)  // correct on multiple round using different correction when some syndromes are still there
	{
	   ...
	   {
	      // correct back if measurement erro on one the 2 surrounding syndroms
	      /*
	      if (__Z10__ && __Z12__)
	      ...
	      */
	      if (__Z10__) qx::pauli_x(1).apply(r);  // correct D1
	      if (__Z4__ ) qx::pauli_x(2).apply(r);  // correct D3
	      if (__Z12__) qx::pauli_x(13).apply(r); // correct D21
	      if (__Z16__) qx::pauli_x(15).apply(r); // correct D25
	   }
	   else if (round==3) // correct for single ancilla syndrom (round 3)
	   {
		 ...
		 qx::pauli_x(2).apply(r); // flip back D3 (corrected on the previous round)
	      }
	      if (__Z12__) 
	      {
		 qx::pauli_x(14).apply(r); // correct D23
		 qx::pauli_x(13).apply(r); // flip back D21 (corrected in the previous round)
	      }
	   }
	}
	 ...
	 */
	void correct_phaseflips(uint32_t round)  // correct on multiple round using different correction when some syndromes are still there
	{
	   qx::qu_register& r = *reg;
	   if (round==1) // correct for 2 ancillas syndrom
	   {
	   ...
	   {
	      // !!!!!! correct back if X44 and X2 still up (might be a measurement error on one of the 2 syndroms)
	      /*
	      if (__X44__ && __X2__)
	      ...
	      */
	      if (__X44__) qx::pauli_z(3).apply(r);  // correct D5
	      if (__X2__ ) qx::pauli_z(1).apply(r);  // correct D1 
	      if (__X14__) qx::pauli_z(9).apply(r);  // correct D15
	      if (__X22__) qx::pauli_z(13).apply(r); // correct D21
	   }
	   else if (round==3) // correct for single ancilla syndrom (round 3)
	   {
	      if (__X2__ ) 
		 ...
		 qx::pauli_z(1).apply(r); // flip back D1 (corrected on the previous round)
	      }
	      if (__X14__) 
	      {
		 qx::pauli_z(15).apply(r); // correct D25
		 qx::pauli_z(9).apply(r); // flip back D15 (corrected in the previous round)
	      }
	   }
	}
	...
	void draw()
	{
	   ...
	   println("                 " << __state(X44));
	   println("     " << __state(D1) << "       " << __state(D3) << "       " << __state(D5));
	   println(" " << __state(Z10) << "       " << __state(X2) << "       " << __state(Z4));
	   println("     " << __state(D11) << "       " << __state(D13) << "       " << __state(D15));
	   println("         " << __state(Z12) << "       " << __state(X14) << "       " << __state(Z16));
	   println("     " << __state(D21) << "       " << __state(D23) << "       " << __state(D25));
	   println("         " << __state(X22));
	}
	...
	void report(std::ofstream& file, std::string title)
	{
	   ...
	   file << "	   <!-- qubits -->\n";
	   file << "\n";
	   file << "	   <circle cx=\"250\" cy=\"50\" r=\"20\" stroke=\"black\" stroke-width=\"1\" fill=\"" << color[X44] << "\" />\n";
	   file << "	   <text x=\"235\" y=\"55\" fill=\"black\">X44</text>\n";
	   file << "\n";
	   file << "	   <circle cx=\"100\" cy=\"100\" r=\"20\" stroke=\"" << data[D1] << "\" stroke-width=\"" << (1+D1) << "\" fill=\"gray\" /> <text x=\"85\" y=\"105\" fill=\"black\">D1</text>\n";
	   file << "	   <circle cx=\"200\" cy=\"100\" r=\"20\" stroke=\"" << data[D3] << "\" stroke-width=\"" << (1+D3) << "\" fill=\"gray\" /> <text x=\"185\" y=\"105\" fill=\"black\">D3</text>\n";
	   file << "	   <circle cx=\"300\" cy=\"100\" r=\"20\" stroke=\"" << data[D5] << "\" stroke-width=\"" << (1+D5) << "\" fill=\"gray\" /> <text x=\"285\" y=\"105\" fill=\"black\">D5</text>\n";
	   file << "\n";
	   file << "	   <circle cx=\"50\" cy=\"150\" r=\"20\" stroke=\"black\" stroke-width=\"1\" fill=\"" << color[Z10] << "\" /> <text x=\"35\" y=\"155\" fill=\"black\">Z10</text>\n";
	   file << "	   <circle cx=\"150\" cy=\"150\" r=\"20\" stroke=\"black\" stroke-width=\"1\" fill=\"" << color[X2] << "\" /> <text x=\"135\" y=\"155\" fill=\"black\">X2</text>\n";
	   file << "	   <circle cx=\"250\" cy=\"150\" r=\"20\" stroke=\"black\" stroke-width=\"1\" fill=\"" << color[Z4] << "\" /> <text x=\"235\" y=\"155\" fill=\"black\">Z4</text>\n";
	   file << "\n";
	   file << "	   <circle cx=\"100\" cy=\"200\" r=\"20\" stroke=\"" << data[D11] << "\" stroke-width=\"" << (1+D11) << "\" fill=\"gray\" /> <text x=\"85\" y=\"205\" fill=\"black\">D11</text>\n";
	   file << "	   <circle cx=\"200\" cy=\"200\" r=\"20\" stroke=\"" << data[D13] << "\" stroke-width=\"" << (1+D13) << "\" fill=\"gray\" /> <text x=\"185\" y=\"205\" fill=\"black\">D13</text>\n";
	   file << "	   <circle cx=\"300\" cy=\"200\" r=\"20\" stroke=\"" << data[D15] << "\" stroke-width=\"" << (1+D15) << "\" fill=\"gray\" /> <text x=\"285\" y=\"205\" fill=\"black\">D15</text>\n";
	   file << "\n";
	   file << "	   <circle cx=\"150\" cy=\"250\" r=\"20\" stroke=\"black\" stroke-width=\"1\" fill=\"" << color[Z12] << "\" /> <text x=\"135\" y=\"255\" fill=\"black\">Z12</text>\n";
	   file << "	   <circle cx=\"250\" cy=\"250\" r=\"20\" stroke=\"black\" stroke-width=\"1\" fill=\"" << color[X14] << "\" /> <text x=\"235\" y=\"255\" fill=\"black\">X14</text>\n";
	   file << "	   <circle cx=\"350\" cy=\"250\" r=\"20\" stroke=\"black\" stroke-width=\"1\" fill=\"" << color[Z16] << "\" /> <text x=\"335\" y=\"255\" fill=\"black\">Z16</text>\n";
	   file << "\n";
	   file << "	   <circle cx=\"100\" cy=\"300\" r=\"20\" stroke=\"" << data[D21] << "\" stroke-width=\"" << (1+D21) << "\" fill=\"gray\" /> <text x=\"85\" y=\"305\" fill=\"black\">D21</text>\n";
	   file << "	   <circle cx=\"200\" cy=\"300\" r=\"20\" stroke=\"" << data[D23] << "\" stroke-width=\"" << (1+D23) << "\" fill=\"gray\" /> <text x=\"185\" y=\"305\" fill=\"black\">D23</text>\n";
	   file << "	   <circle cx=\"300\" cy=\"300\" r=\"20\" stroke=\"" << data[D25] << "\" stroke-width=\"" << (1+D25) << "\" fill=\"gray\" /> <text x=\"285\" y=\"305\" fill=\"black\">D25</text>\n";
	   file << "\n";
	   file << "	   <circle cx=\"150\" cy=\"350\" r=\"20\" stroke=\"black\" stroke-width=\"1\" fill=\"" << color[X22] << "\" /> <text x=\"135\" y=\"355\" fill=\"black\">X22</text>\n";
	   file << "\n";
	   file << "	   </svg> \n";
	   file << "\n";
